<template>
    <AdminLayout title="Гражданство" :breadcrumb="[{label: 'Гражданство'}]">
        ...
        <div class="block">
            ...
            <div class="simple-list-filter-wrp">
                ...
                <Link :href="route('admin.dir-citizenship.create')" class="btn btn-sm btn-primary ">
...
export default {
    ...
    methods: {
        itemClick: function (item) {
            this.$inertia.visit(route('admin.dir-citizenship.edit', {citizenship: item.id}))
        },

        async remove(item) {
            let index = this.items.findIndex(itm => itm.id === item.id);
            let result = await axios.delete(route('admin.dir-citizenship.destroy', {citizenship: item.id}));
