<template>
    <AdminLayout :title="item.id ? item.name : 'Новый регион'"
                 :breadcrumb="[{link: route('admin.dir-region.index'), label: 'Регионы'}, item.id ? item.name: 'Новый']">
...
export default {
    ...
    methods: {
        submit() {
            this.form.errors = [];
            if (this.item.id) {
                this.form.put(route('admin.dir-region.update', {region: this.item.id}));
            } else {
                this.form.post(route('admin.dir-region.store'));
            }
        }
    }
