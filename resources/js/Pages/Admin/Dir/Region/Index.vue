<template>
    <AdminLayout title="Регионы" :breadcrumb="[{label: 'Регионы'}]">
        ...
        <div class="block">
            ...
            <div class="simple-list-filter-wrp">
                ...
                <Link :href="route('admin.dir-region.create')" class="btn btn-sm btn-primary ">
...
export default {
    ...
    methods: {
        itemClick: function (item) {
            this.$inertia.visit(route('admin.dir-region.edit', {region: item.id}))
        },

        async remove(item) {
            let index = this.items.findIndex(itm => itm.id === item.id);
            let result = await axios.delete(route('admin.dir-region.destroy', {region: item.id}));
