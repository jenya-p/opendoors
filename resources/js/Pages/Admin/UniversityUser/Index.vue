<template>
    <AdminLayout title="Представители ВУЗ" :breadcrumb="['Представители ВУЗ']">
        ...
        <div class="block">
            ...
            <div class="simple-list-filter-wrp">
                <input type="text" class="input" placeholder="Поиск по имени и email"
                       v-model.lazy="filter">
                <Link :href="route('admin.university-user.create')" class="btn btn-sm btn-primary ">
            ...
            <table class="table">
                ...
                <tr v-for="item of items.data" @click="itemClick(item)" class="cursor-pointer">
                    ...
                    <td><div class="badges">
                        <div class="role-badge" v-for="(roleName, role) of item.role_names">{{ roleName }}</div></div>
                    </td>
                    <td>
                        {{
...
export default {
    ...
    methods: {
        itemClick: function (item) {
            this.$inertia.visit(route('admin.university-user.edit', {university_user: item.id}))
        },
        async remove(item) {
            let index = this.items.data.findIndex(itm => itm.id === item.id);
            let result = await axios.delete(route('admin.university-user.destroy', {university_user: item.id}));
