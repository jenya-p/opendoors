<template>
    <AdminLayout title="Треки" :breadcrumb="[{label: 'Треки'}]">
...
export default {
    ...
    methods: {
        itemClick: function (item) {
            this.$inertia.visit(route('admin.track.edit', {track: item.id}))
        },

        async remove(item) {
            let index = this.items.findIndex(itm => itm.id === item.id);
            let result = await axios.delete(route('admin.track.destroy', {track: item.id}));
